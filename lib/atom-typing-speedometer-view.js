  ...
  drawElement () {
    if (this.element && this.visibility) {
          ...
          for (let prop in this.data) {
            if (index < this.spectrumElement.children.length) {
              ...
              if (percentage > 0) {
                ...
                this.spectrumElement.children[index].firstChild.style = 'height: 1px;'
              }
            }
            index++
          }
      ...
      if (atom.config.get('atom-typing-speedometer.showSecondAvg')) {
        let countsPerSecondAvg = (countsPerMinute / this.avgCount).toFixed(2)
        this.textElement.textContent += `avg ${countsPerSecondAvg}/s `
      }
